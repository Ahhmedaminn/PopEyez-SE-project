...
function UserManagement() {
  ...
  async function loadUsers() {
    ...
    setUsers(await apiGet(`/users${params.toString() ? `?${params}` : ''}`))
  }
  ...
  return (
    <div className="workspace-section">
      ...
      <div className="two-column">
          ...
          <ul className="list data-list">
            {users.map((user) => (
              <li key={user.id}>
                ...
                <button type="button" onClick={() => updateStatus(user.id, user.status === 'Active' ? 'Deactivated' : 'Active')}>
        ...
        <section className="page-panel">
          <div className="panel-header"><h2>Create Stakeholder</h2></div>
          <form className="form compact-form" onSubmit={createUser}>
            ...
            <input placeholder="Speciality" value={form.speciality} onChange={(e) => updateForm('speciality', e.target.value)} />
